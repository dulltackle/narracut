...
function sha256WithoutWebCrypto(bytes: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma1 =
        ...
        (previous2 >>> 10);
      words[index] =
        (sigma1 + (words[index - 7] ?? 0) + sigma0 + (words[index - 16] ?? 0)) >>> 0;
    }
  ...
  state.forEach((value, index) => resultView.setUint32(index * 4, value));
  return result;
}
...
export async function sha256Utf8(
  text: string,
  cryptoApi: DigestCrypto | undefined = globalThis.crypto,
): Promise<string> {
  const bytes = new TextEncoder().encode(text);
  const digest =
    cryptoApi?.subtle === undefined
      ? sha256WithoutWebCrypto(bytes)

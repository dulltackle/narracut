...
async function assertStream(file: string, codecType: "audio" | "video"): Promise<void> {
  ...
  if (
    !result.streams?.some((stream) => stream.codec_type === codecType) ||
    !Number.isFinite(Number(result.format?.duration))
  ) {
    throw new Error(`缺少可解码的 ${codecType} stream。`);
  }
}
  ...
  for (const entry of referencedRenderMedia(project)) {
    ...
    try {
      if (entry.kind === "image") {
        ...
        await assertStream(file, entry.kind === "speech" ? "audio" : "video");
      }
    } catch {

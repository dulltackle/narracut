...
在 Agent 工作区，桌面主面以 `.82fr / 1.18fr` 并置“临时任务状态”与“验证结果”，底部操作栏固定承载开始验证、停止和继续。在 `900px` 以下，项目检查收窄到 `280px`，Scene/Speech 最右列隐藏，Agent 的任务与结果改为单列；在 `700px` 以下，Project ID 移到独立第二行，连接文字继续可见，项目检查改为从右侧打开的抽屉，Composer 仍固定在底部。移动端接触表保留 Scene、Narration 与 Asset 三列并允许横向内容自然裁切；Agent 工作区以单一纵向滚动容器承载标题、任务状态、验证结果、操作按钮和成片 Preview；内层舞台与状态台自然展开，避免多层滚动截断内容。

成片 Preview 的暗色舞台在桌面使用 `height: min(34vh, 360px)`、最小高度 `220px`，完整保留成片比例；控制区允许换行。在 `700px` 以下舞台使用 `32vh`、最小高度 `180px`，进度控件独占一行，版本、画面和控制沿单列自然滚动。Composer 继续固定，项目检查保持桌面侧栏与移动抽屉的既有位置。

候选检查面板在 Agent 工作区中独立常驻，沿用暗房底色、中文正文与结构线，不改变成片 Preview 主节点。桌面四类门禁采用名称、状态、原因与下一步的三列分隔行；`700px` 以下原因与下一步移到下一行，检查操作与批次选择顺序堆叠。按钮、批次选择和折叠详情入口最小高度为 `44px`，长身份与诊断文本可换行。
...
Agent 工作区的宿主验证区域是单任务状态台，不是聊天界面。顶部只说明固定、只读的宿主任务和协议边界；中部桌面端以任务/结果双列呈现，在 `900px` 以下转为单列。只显示当前任务身份、线程连接、所选 Scene、有界验证结果与最小诊断；不展示 Render Program 源码、对话、推理、工具日志或历史列表。界面必须明说验证不会修改项目内容。状态台之后的成片 Preview 是独立的成片查看主区，与 Asset 文件的只读预览层区分。

### Program Preview

成片主区沿用 Operate 暗房黑与结构线，以固定“当前修订 / 候选”状态对照、单一画面和播放控制组织查看顺序。对照区桌面双列，`700px` 以下纵排并以结构线分隔；同时最多保留两个预览实例。版本名称、持续可见的“正在查看”、描边与 `aria-pressed` 共同确认画面来源。用户显式构建版本，已有画面在构建中保持；目标通过 `READY` 后才提供“已就绪 · 切换查看”，首次展示与切换均从首帧暂停开始，不自动播放。构建失败明确目标与原因，并保留原画面及其来源。

两侧分别显示 Brief、项目输入、Media Revision 与执行环境的真实新鲜度；缺少证据明确显示“状态未知”，不以就绪推断最新。Brief 同时区分已复核、待接受复核与复核状态未知，读取 Brief 不等于复核，重新构建不清除待复核。上一成功候选仍可查看，但过期后持续显示“上一成功预览 · 已过期 · 不能用于接受”；当前修订的过期画面也明确标记来源与不能用于接受。独立琥珀提醒不被失败、定位或缓冲反馈覆盖；Preview 就绪不代表通过全部接受门禁。

显式选择 Scene 时，以稳定 ID 在正在查看版本中查找起始帧并暂停定位；Preview 隐藏时暂存选择，返回后执行。旧版本缺少所选 Scene 时说明无法定位，并引导切换或构建对应版本。定位先显示“正在定位”，仅对应 `FRAME` 确认后更新已提交帧号、时间与播放 Scene；自然播放跨越 Scene 不改变表格选择。没有可播放 Scene 时显示明确空态并隐藏无效控制；使用草稿时间时标明“草稿时间（Draft Duration）”。播放、逐帧、进度、帧号跳转、音量、静音与详情入口目标高度至少 `44px`，焦点使用可见蓝色轮廓。

不可变执行身份与新鲜度证据置于默认收起的原生“预览详情”中。该表面以 `plugins/narracut/workbench-preview.js`、`plugins/narracut/workbench.html` 和 `plugins/narracut/workbench.js` 为实现事实来源。#77 的视觉复核证据为 `.impeccable/review/desktop.png`、`mobile.png`、`mobile-screen.png` 与 `mobile-controls.png`；独立只读审查结论为可交付（`ship`），范围限于该 Preview 表面及相关证据。

### 候选检查与操作状态

检查面板延续 Operate 工作台，以“对象与新鲜度 → 四类操作门禁 → 诊断与主观警告 → 默认折叠的阶段及身份”组织阅读。门禁分别显示候选 Preview、候选交付、接受候选和最终 Render 的状态、原因与下一步；四者独立判定，接受候选与最终 Render 当前持续标明“尚未启用”。完成检查或没有内容诊断都不能代替门禁结论。

最多保留具名的最新与上一批次，切换只改变所查看的诊断；门禁始终表达最新状态。新鲜度按完整身份判定，身份变化使整批过期，旧结果只供参考，不能与新批次拼接。异步返回的身份不符结果不会并入本批次，并明确记录未运行原因。检查刷新、批次切换与取消均保留正在查看的 Preview 及其主节点。

每条诊断依次提供问题、阶段与稳定代码、主位置、修复建议、影响操作，再以原生详情折叠身份与相关位置；主观警告单独标明“主观建议 · 不阻断”。有界结果必须展示截断提示及未展示数量，主观警告未全部展示也明确说明不能用于交付。阶段分别显示等待、运行、通过、有问题或未运行及其原因，不合并成单一成功灯。

文字徽标以小形状补充语义：绿色圆点表示可用或通过，蓝色圆点表示运行，琥珀方形表示问题，空心方形表示等待或尚未启用。徽标样式与原项目检查圆点隔离，避免旧圆点尺寸裁切中文状态。折叠详情、批次选择和位置按钮均可通过键盘操作并显示蓝色焦点轮廓；局部刷新保留详情展开与可恢复焦点。Scene 定位使用明确按钮；frame 仅为时间导航上下文，不推断对应 Scene 或修复对象。

...
成片主区沿用暗房黑与结构线，不添加纸面或发光卡片。标题与版本按钮使用可读名称区分当前版本和候选，正在查看的版本同时显示文字标记、描边与 `aria-pressed`。用户显式构建当前版本或候选；已有画面在构建时保持，目标通过 `READY` 后才提供“已就绪 · 切换查看”。首次展示与切换均从首帧暂停开始，不自动播放。

“当前显示上一份成功预览，已过期”是独立、持续可见的琥珀文字状态，不被构建失败、定位或缓冲的临时反馈覆盖。播放、逐帧、进度、帧号跳转、音量、静音与详情入口的目标高度至少 `44px`，焦点采用可见蓝色轮廓。没有可播放 Scene 时明确说明并隐藏无效控制。帧号、时间和播放 Scene 只在对应 `FRAME` 确认后更新；定位请求先显示“正在定位”，不把请求位置冒充已提交位置，也不改变表格所选 Scene。

不可变执行身份置于原生 `details` 的“预览详情”中，默认收起；正常查看顺序是版本、状态、画面、播放控制与已确认位置。该表面由 `plugins/narracut/workbench-preview.js` 和 `plugins/narracut/workbench.html` 实现。#76 的视觉复核证据为 `.impeccable/review/desktop.png`、`mobile.png` 与 `mobile-controls.png`；原三项问题均已解决，视觉交付结论只覆盖这些截图与相关 Preview 代码。

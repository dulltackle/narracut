...
视频主面上方原位呈现更新状态和“仅同步表格内容”，说明保持独立标题、图表、动画规则。无设计时禁用并引导当前对话生成；进行中显示真实阶段与取消，未知回执提供核对入口。表格变化后立即停播遮住旧画面，成功才显示完整视频。更新摘要、常驻草稿警告、最近操作及撤回说明相邻，检查证据按需展开。

此路径替代上述候选比较、接受和多版本历史的正常入口；内部成果仍在详情中提供兼容接续／完整性恢复。独立输出读取成功发布证据，撤回后必须重新同步。按钮最低 44px，白色工作面、深灰主按钮、自然换行；异步完成不抢焦点。生产证据范围见 #122 验收记录。

### 独立画面撤回（#128）

沿用浅色工作台，视频附近以轻分隔线分开展示内容同步与最近画面创作。后续同步不改变创作摘要、时间与撤回目标；按钮为“撤回首次生成”或“撤回上次画面调整”。原位确认说明保留最新表格及 Speech、恢复后自动同步与不能重做，默认聚焦取消，取消回到入口。表格工具栏明确显示“撤销表格编辑”。

确认后立即遮住旧视频。恢复事实与同步阶段分别表达；同步失败／取消保留恢复结果，提供“重试同步”，未知回执先核对。无设计时引导当前 Codex 对话生成。状态持续显示，异步完成不抢焦点；窄屏自然换行，所有操作至少 44px。证据与实际执行范围见 [#128 验收](docs/acceptance/issue128/README.md)。

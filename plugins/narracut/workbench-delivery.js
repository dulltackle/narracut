...
function createDeliveryWorkbench(call, getProject, preview, navigateScene) {
  ...
  function update() {
    ...
    for (const problem of batch?.diagnostics ?? []) if (problem.severity === 'error') missing.push(`${problem.message}${problem.suggestion ? ' · ' + problem.suggestion : ''}`);
    for (const gate of data.checks?.gates ?? []) if (['preview', 'delivery', 'accept'].includes(gate.operation) && gate.status === 'blocked' && gate.reason) missing.push(gate.reason);
    content(region.querySelector('[data-delivery-missing]'), [...new Set(missing)].map(message=>`<li>${escape(message)}</li>`).join(''));
    ...
    content(region.querySelector('[data-delivery-suggestions]'), `<h3>Scene 修改建议</h3>${report?.suggestions.length ? report.suggestions.map((item,index)=>`<article><strong>${getProject()?.scenes?.some(scene => scene.id === item.sceneId) ? `Scene ${getProject().scenes.findIndex(scene => scene.id === item.sceneId) + 1} · ` : ""}${escape(item.action)} · 可选优化</strong><code>Scene ID · ${escape(item.sceneId)}</code><p>${escape(item.observation)}</p><p>建议内容：${escape(item.content)}</p><p>理由：${escape(item.reason)}</p><button data-copy-suggestion="${index}" data-focus="copy-${index}">复制建议值</button><button data-go-scene="${index}" data-focus="scene-${index}" ${getProject()?.scenes?.some(scene => scene.id === item.sceneId) ? '' : 'disabled'}>前往表格定位 Scene</button>${getProject()?.scenes?.some(scene => scene.id === item.sceneId) ? '' : '<p>目标 Scene 已删除，无法定位；建议内容仍可复制。</p>'}</article>`).join('') : `<p>${report ? '当前报告没有 Scene 修改建议。' : '等待 Agent 提交建议；建议不会直接修改 Scene。'}</p>`}`);
    ...
    if (summary) summary.textContent = report?.summary ?? (getProject()?.hasCandidate ? '候选正在准备 · 等待变更摘要。' : '尚无候选 · 请在当前 Codex 对话中描述创作目标。');
    const notices = document.querySelector('[data-review-notices]');
    if (notices) content(notices, `<p>${escape(state.textContent)}</p>${[...new Set((batch?.diagnostics ?? []).filter(item => item.severity === 'error').map(item => item.message).concat((data.checks?.gates ?? []).filter(gate => gate.status === 'blocked').map(gate => gate.reason).filter(Boolean)))].map(message => `<p>${escape(message)}</p>`).join('')}<div>${region.querySelector('[data-delivery-warnings]').innerHTML}</div>`);
    frames();
  ...
  function focusRegion(selector) { if (selector === '[data-preview-state]') document.querySelector('[data-close-review]')?.click(); const target = document.querySelector(selector); if (!target) return; target.closest('details')?.setAttribute('open', ''); target.tabIndex = -1; target.scrollIntoView({block:'start'}); target.focus({preventScroll:true}); }
  ...
  return {
    ...
    mount(node){
      ...
      region.innerHTML='<header><h2>候选交付</h2><strong data-delivery-state role="status" aria-live="polite">尚未齐备</strong></header><div data-delivery-summary></div><ul data-delivery-missing></ul><div class="delivery-actions"><button data-delivery-preview>查看候选 Preview</button><button data-delivery-prepare>准备候选证据</button><button data-delivery-retry hidden>重试失败项</button><button data-delivery-checks>查看检查详情</button></div><p data-delivery-progress></p><p data-delivery-build-proof hidden></p><div class="delivery-report"><section data-delivery-warnings></section></div><details data-evidence-details><summary>展开代表帧证据</summary><div data-delivery-frames></div></details><details class="checks-details"><summary>详细检查与操作状态</summary><section class="checks-panel" data-program-checks aria-label="检查与操作状态"></section></details><section data-delivery-suggestions></section><details><summary>完整覆盖计划与身份</summary><div data-full-plan></div></details><p class="delivery-footnote">代表帧检查不记录用户观看范围，也不自动判定审美质量。接受不代表最终 Render 已完成。</p>';
      region.addEventListener('click',event=>{
        ...
        if(button.hasAttribute('data-copy-suggestion')){const suggestion=current().report.suggestions[Number(button.dataset.copySuggestion)];void navigator.clipboard.writeText(suggestion.content).then(()=>{button.textContent='已复制';}).catch(()=>{button.textContent='复制失败，请选择文本复制';});}
      });

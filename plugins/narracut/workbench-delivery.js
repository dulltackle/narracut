...
function createDeliveryWorkbench(call, getProject, preview, navigateScene) {
  ...
  function update() {
    ...
    content(region.querySelector('[data-delivery-suggestions]'), `<h3>Scene 修改建议</h3>${report?.suggestions.length ? report.suggestions.map((item,index)=>`<article><strong>${getProject()?.scenes?.some(scene => scene.id === item.sceneId) ? `Scene ${getProject().scenes.findIndex(scene => scene.id === item.sceneId) + 1} · ` : ""}${escape(item.action)} · 可选优化</strong><code>Scene ID · ${escape(item.sceneId)}</code><p>${escape(item.observation)}</p><p>建议内容：${escape(item.content)}</p><p>理由：${escape(item.reason)}</p><button data-copy-suggestion="${index}" data-focus="copy-${index}">复制建议值</button><button data-go-scene="${index}" data-focus="scene-${index}">定位 Scene</button></article>`).join('') : `<p>${report ? '当前报告没有 Scene 修改建议。' : '等待 Agent 提交建议；建议不会直接修改 Scene。'}</p>`}`);

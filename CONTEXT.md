...
**Agent 工作区**：
用于查看 Agent 创作任务状态、同步最新表格内容、查看单一完整视频和撤回最近一次视频更新的工作区；不提供聊天输入或发起创作功能，创作对话使用 Codex 当前对话。
...
**Agent 创作任务状态**：
任务的四种稳定状态：运行中、等待用户、已停止和已终结；它们描述创作任务，不证明 Codex 此刻仍在执行。状态与具体原因分别表达，等待 Agent 返回结果不等于等待用户，等待用户期间没有后台 Agent 动作。
...
**候选 Render Program**：
从当前 Render Program 派生的隔离工作版本，是 Agent、人工和外部工具修改程序的唯一可写对象；每个项目同时最多一个，完整更新发布后才成为当前修订；存量候选可保留用于创作接续或完整性恢复。
_Avoid_: 草稿分支、临时补丁

**Render Program 修订**：
完整视频更新成功发布后形成的完整、不可变 Render Program，具有稳定身份并关联发布时的程序与输入证据；字节被改写属于损坏，不是新修订。
_Avoid_: 备份、可变版本
...
**视频更新撤回**：
撤销最近一次成功生成或调整的画面设计，保留最新表格与 Speech，并持久消费这一步；恢复已有设计后自动同步最新内容，首次生成撤回后回到无设计。内容同步不占用画面撤回，恢复后同步失败只重试同步，不连续回退或重做，也不展示旧内容视频。
_Avoid_: 表格撤销、播放旧视频、Render Program 回退

**视频更新发布**：
把通过最新输入检查的完整视频状态原子生效的操作；生成或调整同时建立对应一步画面撤回，内容同步保留既有撤回能力与程序语义，发布不代表最终视频输出完成。
_Avoid_: 自动接受、逐文件覆盖、部分更新

**Render Program 接受**（存量兼容协议）：
...
**候选 Preview**：
候选 Render Program 成功构建后产生的不可变预览产物；内部成功产物只有完成发布且对应最新输入时才能在单一视频主面查看；过期结果停止展示。
_Avoid_: 实时源码预览、自动发布
...
**Speech 试听**：
对单个 Scene 有效 Speech 本体的只读听取，用于检查该句声音；它与成片 Preview 分开，不改变 Scene 内容或成片时间。
_Avoid_: Scene 预览、成片预览、音色试生成

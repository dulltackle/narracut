...
**Codex 创作线程**：
承载一项 Agent 创作任务的专用 Codex Thread；同一任务可以在其中经历多个 Turn，但同一时刻只由一个 Thread 驱动，也不与其他 Agent 创作任务共用对话上下文。原线程不可用时，同一任务可以依据 Narracut 保存的最小任务检查点、候选 Render Program 与最新项目内容显式改绑到替代线程；同一项目从另一个 Codex Thread 打开时则自动中断旧 Turn、撤销旧 Thread 的任务写权并把任务改绑到新 Thread。线程记录不是随项目移动的权威来源。
...
**Agent 任务恢复**：
用户明确继续一项已停止 Agent 创作任务时，以一次操作恢复可用的原 Codex 创作线程，或在其不可用时自动创建替代线程，再从 Agent 任务检查点、当前候选与最新项目内容开始新的 Turn。恢复只承认最后一个原子持久化边界，丢弃并按需重跑中断时尚未提交的修改、工具调用和中间判断；检查点缺失、损坏或与候选不一致时，原任务不可恢复，但候选和其他项目内容不受影响，用户仍可明确让新任务接管现有候选。

...
**Agent 创作任务**：
用户明确发起的一次有边界的自主创作循环；Agent 在任务内持续读取最新输入、修改候选 Render Program、运行快速检查、构建候选 Preview 并检查时间线代表点，直到候选就绪、需要用户决策或进入停止状态，不作为脱离任务的后台创作进程。用户停止、Codex 中断或应用重启只使任务停止运行并保留继续同一任务所需的最小状态；接受或放弃候选，以及用户明确以新目标取代原任务，才终结该任务。任务期间相关项目输入变化时，Agent 自动作废过期执行结果、读取最新值并继续，不要求用户确认这次追赶；切换工作区不停止任务。
_Avoid_: 后台 Agent、常驻 Agent

**Codex 创作线程**：
承载 Agent 创作任务的当前 Codex Thread；同一任务可以在其中经历多个 Turn，同一 Thread 允许顺序承载多个任务，各任务以独立任务检查点与候选状态区分，同一时刻只有一个任务拥有候选写权，且同一任务只由一个 Thread 驱动。同一项目从另一个 Codex Thread 打开时仅查看；只有用户在新对话明确发起或继续创作，才中断旧 Turn、撤销旧 Thread 写权并转移到当前 Thread，继续同一任务以任务检查点、候选与最新项目内容为依据。线程记录不是随项目移动的权威来源。
_Avoid_: 项目聊天记录、永久项目线程、Render Program 历史

**Agent 任务检查点**：
随项目原子替换、只用于停止后继续同一 Agent 创作任务的单一当前状态；它保存任务 ID、稳定状态与原因、当前创作指令、相关项目和 Render Program 身份、最后完成的安全阶段、待用户处理事项，以及一个允许失效的 Codex 创作线程指针。检查点只引用已经独立持久化的候选修改、结构化诊断与候选 Preview 身份，不保存原始工具输出、日志、未提交修改、推理、中间计划、Agent 总结、模型身份、token 用量或活动时长，也不形成任务历史。任务终结时检查点删除，Codex Thread 仍由 Codex 自身管理；正式复制项目时，可移动状态和当前创作指令进入使用新任务 ID、清除 Thread 指针的停止状态检查点。
_Avoid_: Agent 对话备份、任务事件日志、第二份 Render Program 历史

**Agent 创作任务状态**：
Agent 创作任务只有运行中、等待用户、已停止和已终结四种稳定状态，具体原因由含义稳定的原因代码另行说明。等待工具审批、用户判断或 Scene 修改时没有后台 Agent 动作；批准工具调用或完成任务所等待的 Scene 修改会直接触发同一任务继续，候选就绪、普通用户判断或外部候选变化仍需用户明确选择。应用重启、用户停止、Codex Thread 丢失、账户额度阻断，以及连续多轮没有形成新的持久成果使任务进入已停止，必须由用户明确继续；Narracut 不另设任务级 token、活动时间或迭代次数上限。接受或放弃候选，以及新目标显式接管既有候选后取代旧任务，使旧任务进入已终结。
_Avoid_: 每原因一种状态、自由文本状态、后台等待

**Agent 任务恢复**：
用户明确继续一项已停止 Agent 创作任务时，由当前 Codex 对话承载任务，必要时撤销旧线程写权并改绑，再依据 Agent 任务检查点、当前候选与最新项目内容恢复执行，不自动切回原线程或创建替代线程。恢复只承认最后一个原子持久化边界，丢弃并按需重跑中断时尚未提交的修改、工具调用和中间判断；检查点缺失、损坏或与候选不一致时，原任务不可恢复，但候选和其他项目内容不受影响，用户仍可明确让新任务接管现有候选。
_Avoid_: 进程恢复、模型内存恢复、从聊天记录猜测任务状态

**当前创作指令**：
用户发起或继续 Agent 创作任务时明确表达的本次意图，由初始指令及后续明确的用户创作修订原文按顺序组成，后来的指令覆盖与之冲突的早期指令；Composer 中含义明确的创作要求自动追加，问题、状态询问、工具审批回答和闲聊不追加，混合或含糊消息只有在用户确认拟保存的原文片段后才追加。在成片表现上它优先于 Video Brief 和现有 Render Program，但不能推翻 Scene 内容、时间、Speech、确定性或其他系统硬约束。Agent 必须指出它与 Video Brief 的实质分歧，不得据此静默修改 Brief。
...
**Render Program 接受**：
用户把已经对应最新项目输入并通过阻断检查的完整候选提交为新修订的原子动作；接受确认的是候选 Preview 所代表的完整视频状态，而不是孤立的 Render Program 源码。Video Brief、Scene、Asset、Speech 或执行环境变化后，当前修订仍保持当前，但不再是已针对最新状态接受，必须重新形成候选并接受后才能最终 Render。提交点同时选定新当前修订并消费候选，此前失败保留旧当前与候选，此后的历史裁剪和遗留文件清理不撤销接受。
...
**Render Program 诊断**：
对特定 Render Program、项目输入和执行环境下已观察事实的稳定结构化描述；它包含含义稳定的问题代码、检查阶段、相关身份、面向用户的说明与修复建议，以及一个能够证明的主要位置和可选相关位置，但不猜测无法证明的关联，也不直接决定哪些操作可用。帧所在的 Scene Time Window 只提供导航上下文，除非检查能够直接证明，否则不表示问题属于该 Scene。主观视觉质量不是系统可以判定的诊断事实，Agent 可以提出非阻断警告，最终由用户判断。
_Avoid_: 门禁结果、验收结论、自动审美评分

**Render Program 门禁**：
根据最新 Render Program 诊断与验收证据，分别决定候选 Preview、候选交付、Render Program 接受和最终 Render 是否可用的判断；不同操作可以得到不同结论，任何违反系统硬约束的阻断都不能由用户忽略。最终 Render 新发现的内容或确定性失败可以阻断一个已接受修订再次 Render，但不撤销已经发生的 Render Program 接受；执行胶囊、进程、编码器或输出位置等操作失败不改变接受事实，条件恢复后可以重试同一完整视频状态。
...
**Render Program 验收记录**：
Render Program 接受时随新修订保存的精简、不可变记录，关联完整视频状态身份、检查器与协议版本、各阶段结果、非阻断警告、代表帧计划与图像摘要以及最终门禁结论；它记录接受时检查过什么，不保存 Bundle、完整日志或代表帧图片，也不保证变化后的状态仍可执行。
_Avoid_: Bundle 缓存、Preview 录屏、用户观看记录

**Render Program Bundle**：
固定工具链在特定执行环境中从一份完整 Render Program 构建出的不可变派生产物，以完整字节指纹标识；候选 Preview 与最终 Render 必须使用验收记录所绑定的同一 Bundle 身份。Bundle 不属于 Render Program 修订或可移动项目内容；缓存缺失时可以在相同执行环境重建，但只有指纹完全相同的结果仍是已接受的 Bundle。
_Avoid_: 源码目录、Render Program 修订、可变开发服务器

**Render Program 检查批次**：
针对同一 Render Program、项目输入和执行环境身份运行的一组分阶段检查结果；互不依赖的阶段可以继续收集诊断，缺少前置条件的下游阶段记为未运行而不制造额外诊断。任一相关身份变化都会使整批结果过期，过期结果不能与新批次混合。
...
**恢复截面**：
项目身份失效时立即冻结用户输入、再与已经开始的原子提交对账后封存的一组一致状态，包含用户尚未安全落盘的内存编辑及其最后安全持久基线身份；未提交的 Agent 修改批次不进入其中。提交结果暂时不可读时可以保留提交前与拟提交两种精确结果，恢复来源必须命中其一才能消除歧义；截面封存后的后台结果或继续输入不能改变它，随后每次导出恢复快照都读取它。
_Avoid_: 实时恢复状态、Agent 工作副本

**恢复基线**：
恢复截面用于绑定可读原项目持久内容的精确字节证据，由原 Project ID，以及 DSL、Video Brief、当前修订、候选、候选恢复检查点、Render Program 历史索引和离线依赖库各自的身份与指纹组成；正在对账的原子提交可以暂时为单个组件记录提交前与拟提交两个允许指纹。恢复必须逐项验证并消除任何二选一结果，以便把外部变化或损坏定位到具体项目内容，而不是用语义等价或一个总指纹掩盖差异。
_Avoid_: 项目备份、单一项目校验和

**恢复来源**：
恢复时用于提供已持久项目内容的、由用户明确指定的可读内容目录；恢复快照中的原绝对路径只是默认定位提示，逐项恢复基线精确匹配才证明它是可用来源。目录的项目清单缺失或损坏时，项目恢复可以用快照冻结的原 Project ID 重建清单；任何能够可靠读出的不同 Project ID、不同项目格式或其他基线差异都会拒绝来源，应用也不扫描磁盘寻找替代目录。
_Avoid_: 最近项目、自动发现的副本、备份目录

**恢复快照**：
从恢复截面导出到项目外、绑定恢复基线的单文件增量资产；它只携带脏 DSL、未保存 Video Brief 和验证所需元数据，不复制 Render Program 源码、Asset、Speech、其他已持久项目内容或未原子提交的 Agent 修改。Video Brief 已处于三方冲突时可以额外携带 BASE 字节，但它只是不落入项目的冲突证据；恢复仍必须以可读且匹配的原项目持久内容在新路径重建，不能合并到不同 Project ID 的项目，原持久内容已经丢失时明确拒绝恢复。
_Avoid_: 渲染快照、备份

**恢复计划**：
恢复快照与恢复来源全部通过只读验证后形成的待确认差异视图，明确将恢复的 DSL 与 Video Brief、保持不变的 Render Program 状态、不会来自快照的持久项目内容及所有阻断问题。恢复计划不创建或修改目标项目，也不尽力解释损坏或不兼容的载荷。
_Avoid_: 恢复预览项目、部分恢复结果

**项目恢复**：
用户以恢复快照和精确匹配的恢复来源，在不存在的新路径原子重建原 Project ID 的有限操作；它复制来源当前且在复制期间保持稳定的持久项目内容，只应用快照中的用户内存成果，并在发布前解决 Video Brief 冲突。恢复快照不冻结 Asset、Speech 或历史 Render 产物的身份；这些文件在复制期间发生变化会中止操作，复制成功后的当前字节则按既有规则使相关 Preview 与验收证据过期。恢复的 DSL 与 Brief 成为正常已保存内容并使不再对应最新输入的 Render Program 重新等待验收，但项目恢复不创建 Render Program 修订或项目级恢复日志；操作全有或全无，成功后也不消费、修改或删除恢复快照。
_Avoid_: 合并项目、正式复制、续传恢复

**恢复载荷导出**：
完整项目恢复被来源问题阻断时，由用户把已经独立通过完整性与内容校验的脏 DSL、Video Brief LOCAL 或冲突 BASE 分别导出为项目外普通文件的最后抢救手段。导出物不是项目、不能自动导入或覆盖项目，也不允许从版本不兼容、信封损坏或自身校验失败的载荷中尽力提取。

...
同一项目已由另一 Codex 对话控制时，新对话中的面板只读，显示“项目由另一对话控制”，禁用全部写操作，包括 Scene 编辑、接受与放弃候选。用户在当前 Codex 对话明确要求接管、发起或继续创作后，才按线程写权转移规则开放写操作；仅打开或重开面板不能接管。接管本身不等于发起新创作任务，也不隐式接受或放弃候选。旧对话失去写权后，其面板同样进入只读；写权约束必须在服务端执行，不能只靠按钮禁用。
...
Asset 预览只检查 Scene 当前引用且登记表中存在的 Asset 本体，不包含 Subtitle 或其他成片表现，也不改变 Scene 选择或 Player 状态。已知格式尽力预览，未知格式显示通用文件信息；磁盘文件不可用时保留检查入口并显示不可用状态，悬空 Asset ID 没有可预览对象。
...
存在的 Speech 必须使用固定项目相对路径 `speech/<sceneId>.mp3`，并与该 Scene ID、Narration 原始 UTF-8 字节摘要、当前 TTS profile 和音频内容 SHA-256 匹配。新生成记录必须写入 `audioContentHash`；缺少该字段的历史记录仅能按 Draft 使用，重新生成后升级。音频被原位替换时，只把该 Scene 标为不可用于最终 Render，不改变 Scene 身份、Narration 或 Asset 引用。
...
- `undo` 为 null 或最近成功画面创作的 `requestId/kind/at/previousRevisionId/revisionId`；只有 generate／adjust 建立或替换记录，先前无设计时 `previousRevisionId` 为 null。sync 只更新有效视频证据，保留原撤回身份、摘要、时间和目标，不创建、替换或消费画面撤回。
- `result` 保存最近发布或撤回的 `requestId/operationId/status` 供幂等核对。成功发布前完整验证程序和输入，失败或取消保持原指针与撤回记录。

撤回只恢复先前设计身份、持久消费一步记录，保留最新表格、Speech 和内部成果。恢复提交与后续内容同步是两次独立操作：有设计时自动检查并同步最新输入，成功前不展示旧内容视频；无设计时停在无设计状态，不补建 starter。同步失败或取消保留恢复事实，只提供重试同步，不再次撤回，不连续回退或重做。只有后续成功画面创作替换撤回记录。修订保留完整树和元数据绑定；20 份有限索引固定保留当前修订、可撤回创作及先前设计目标，其余位置保留最近修订，避免连续同步挤掉恢复目标。正常页面没有多版本管理入口。暂存的未发布修订可保留为内部成果，不能成为可见视频。

`restoration` 独立保存已消费撤回的 requestId、operationId、后续 syncRequestId 和 synced；同步发布后仍可核对原撤回回执。`undo` 请求携带 parentOrigin，页面 status 在恢复未同步且无在途工作时可携带该来源接续同步；普通状态读取不发起执行。撤回提交后的中断通过持久记录核对，不能重新应用撤回。

存量 sync 记录不转为画面创作身份；存量 generate／adjust 必须核对绑定修订中的请求、来源、时间及先前目标。不足以验证时禁用撤回并显示“无法确认可撤回的画面创作”，详情说明原因，后续同步保留该说明。不删除存量候选、恢复检查点或历史字节来伪造迁移。

工作台分开展示“内容同步”和“最近画面创作”。撤回按钮命名为“撤回首次生成”或“撤回上次画面调整”，原位确认默认聚焦取消，保留表格和 Speech 的后果明确说明。表格操作为“撤销表格编辑”。撤回立即遮住旧视频，异步结果不抢焦点；恢复失败、取消及未知回执提供常驻说明与核对／重试入口。
...
每项 Agent 创作任务由当前 Codex 对话作为 Codex 创作线程承载，可跨多个 Turn。同一个 Codex 对话允许顺序承载多个创作任务，以独立任务检查点和候选状态区分任务；同一时刻只有一个任务拥有候选写权，且只能有一个线程拥有该任务的写权。另一 Codex 线程打开同一项目时仅查看，不中断旧 Turn 或转移写权。只有用户在新对话中明确发起或继续创作，Narracut 才中断旧 Turn、撤销旧线程写权，并把相应任务写权转移到当前线程；发起新任务仍遵守既有候选接管与任务终结规则。
...
- 已停止状态一律需要用户明确继续；用户在当前 Codex 对话中继续时，由当前对话承载恢复后的任务。

接受或放弃候选终结任务；用户用新目标明确接管现有候选时终结旧任务、建立新任务 ID 并保留候选字节。

### 8.2 当前创作指令与检查点

当前创作指令由用户原文按顺序组成；后来的明确创作修订覆盖冲突的早期要求。Composer 中含义明确的创作要求自动追加；问题、状态询问、工具审批回答和闲聊不追加，混合或含糊消息必须先让用户确认拟保存片段。
...
项目原子保存单一 Agent 任务检查点，只包含任务 ID、稳定状态与原因、当前创作指令、相关项目/候选/输入身份、最后完成的安全阶段、待用户处理事项和可失效的 Codex 创作线程指针。检查点只引用已经独立持久化的候选修改、结构化诊断与候选 Preview 身份。不得保存原始工具输出、日志、未提交修改、推理、计划、Agent 总结、模型、token、活动时长或任务历史。任务终结时删除检查点，Codex Thread 仍由 Codex 自身管理。

应用重启后任务保持已停止。用户在当前 Codex 对话中明确继续时，Narracut 使用当前对话承载任务；若原绑定指向另一线程，先撤销旧线程写权，再依据最后原子检查点、当前候选与最新项目内容恢复执行，不自动切回原线程或创建替代线程。未提交修改与工具调用一律丢弃并按需重跑；不得从聊天记录猜测状态。检查点缺失、损坏或与候选不一致时，原任务不可恢复，但候选与项目其他内容保持可用，新任务可以明确接管候选。
...
恢复不恢复 Undo/Redo，不移动当前修订，不创建新修订，不改变候选/历史关系。相关输入变化只使 Preview 与验收证据过期。成功后恢复快照保持原样，不消费、不修改、不删除。恢复快照不冻结 Asset、Speech 或历史 Render 产物的身份；这些文件在复制期间变化会中止操作，成功后按当前字节核对相关 Preview 与验收证据的新鲜度。
...
14. 完整工作台在当前 Codex 对话右侧打开，创作仅使用当前对话，候选审阅界面没有聊天输入或发起创作功能；面板打开失败显示原因并支持重试，不自动打开外部浏览器。
15. 关闭与重开面板不停止创作或隐式接管；另一对话打开同项目时全部写操作受只读约束，明确接管后新旧面板写权同步改变，旧写请求不能提交。顺序任务与当前对话中的恢复遵守独立任务检查点和唯一候选写权。

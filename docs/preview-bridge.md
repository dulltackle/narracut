...
同时最多保留两个浏览器槽位。新目标就绪只展示切换入口；显式切换先暂停源，再显示已 READY 目标并定位首帧。构建或目标初始化失败不移除源画面，错误明确命名目标版本；旧成功预览显示过期提示。零 Scene 没有时间线和播放能力，草稿时间明确标记 Draft Duration。候选接受、代表帧审核和最终 Render 不在本票范围。

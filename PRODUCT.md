...
当前插件表面的承诺更窄：用户先通过启动器原子创建空 Project VNext，或严格打开已有有效项目，再在表格工作区新增、编辑、复制、重排和删除 Scene，并通过可观察的自动保存、Undo/Redo、冲突与身份失效状态确认内容安全。Asset、Speech、Agent、Preview、Render Program 与 Composer 仍没有 Scene 写能力；界面不把尚未实现的生成或播放能力伪装成可用功能。
...
当前表格工作区把 Scene 表现为接触印样中的连续剪接行：Narration 是可原位编辑的主内容，Asset 与 Speech 只呈现身份、相对路径或状态，不以缩略图画廊主导阅读。接触表上沿的操作轨承载新增、复制、重排、删除、Undo/Redo 与项目级保存状态。Agent 工作区继续提供固定、只读的 Codex 创作线程宿主验证；Composer 保留稳定位置并诚实禁用。
...
- 当前共有十个工具：连接检查、启动器、项目创建、项目打开、项目检查、仅 app 可见的 Scene 保存，以及宿主验证的开始、状态读取、停止、继续。Scene 保存只接受持有当前项目租约的工作台，并以 Project ID 与磁盘基线摘要拒绝身份变化或外部冲突；Agent 看不到这一 app 专用写入口。
...
- 表格工作区可新增、编辑、复制、重排和删除最多 1,000 个 Scene；修改 Narration 立即移除失效 Speech，复制生成新 Scene ID 且不复制 Speech。Asset、Speech 生成、Preview、Render、候选管理和自由 Agent 创作任务不在当前交付内。

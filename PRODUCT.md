...
- 完整工作台通过 Narracut Codex 插件在当前 Codex 对话右侧面板中打开，使用本地 Project VNext 项目；创作对话与输入使用 Codex 当前对话及其 Composer。领域术语以 `CONTEXT.md` 为准。
- 表格工作区承载 Narration、Asset、Speech 等 Scene 内容的检查与编辑；Agent 工作区仅承载任务状态、候选交付、Preview 审阅及接受或放弃操作，不提供聊天输入或发起创作功能。
...
- 普通停止、应用重启或 Codex 中断保留候选与最小任务检查点；恢复需要用户明确继续。切换工作区或关闭右侧面板不停止任务；重新打开面板展示最新状态。另一 Codex 对话打开同一项目时仅查看，明确发起或继续创作时才转移写权。
...
- 右侧面板打开失败时显示原因并允许重试，不自动跳转外部浏览器。项目由另一对话控制时，面板标明只读并禁用全部写操作，用户在当前对话明确接管后才开放。
